...
class ISM(BaseAlgorithm):
    ...
    @torch.no_grad()
    def compute_ism(self, latents, noise, t: torch.Tensor) -> dict:
        bsz = latents.shape[0]
        ...
        x_s = self.scheduler.add_noise(latents, noise, timesteps=t_init)
        for i in range(n):
            e_s = predict_noise_no_cfg(
                unet=self.unet,
                noisy_latents=x_s,
            ...
            x_s, x_s_0 = ddim_inverse_step(
                ...
                e_t=e_s,
                t=t_init + i * self.cfg.delta_s,
                delta_t=self.cfg.delta_s,
            )

        # 2. get x_t using one single extra ddim inv step from x_s
        e_s = predict_noise_no_cfg(
            unet=self.unet,
            noisy_latents=x_s,
            text_embeddings=self.null_embed.expand(bsz, -1, -1),
